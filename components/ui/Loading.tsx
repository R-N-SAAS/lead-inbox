...
export function Spinner({ size = 'md', color = 'primary', className }: SpinnerProps) {
  ...
  const colors = { primary: 'text-blue-500', white: 'text-slate-900', slate: 'text-slate-400' };

  return (
    <svg className={cn('animate-spin', sizes[size], colors[color], className)} fill="none" viewBox="0 0 24 24">
...
export function LoadingOverlay({ message = 'Laden...', fullScreen = false }: { message?: string; fullScreen?: boolean }) {
  return (
    <div className={cn('flex flex-col items-center justify-center bg-white/80 backdrop-blur-sm', fullScreen ? 'fixed inset-0 z-50' : 'absolute inset-0 rounded-2xl')}>
      <Spinner size="lg" />
